import React from "react";
import "./style.css";
import { Col, Row } from "antd";

function Section5() {
  return (
    <div>
      <p
        style={{
          textAlign: "center",
          color: "#003da5",
          fontSize: "32px",
          fontWeight: "700",
        }}
      >
        P&G hỗ trợ Save Ralph
      </p>
      <p
        style={{
          margin: "auto",
          textAlign: "center",
          fontSize: "16px",
          width: "50vw",
        }}
      >
        P&G hỗ trợ #SaveRalph, chiến dịch kêu gọi loại bỏ thử nghiệm mỹ phẩm
        trên động vật trên toàn cầu của Tổ chức Nhân đạo Quốc tế
      </p>
      <img
        src="https://images.ctfassets.net/ywowj8d94i8y/56ROYQNXM1BFnxD8Dk1pQ4/0da5a5698bcf5047547b7e6f7dba4f4f/RALPH_shot01_01_X1_02991__1_.jpg?fm=webp"
        alt=""
        style={{ margin: "auto", display: "flex", paddingTop: "50px" }}
      />
      <button class="btnSeeMore">
        <span class="btn-transition"></span>
        <span class="btn-label">
          <p>Tìm hiểu thêm </p>
          <i class="gg-arrow-top-right"></i>
        </span>
      </button>

      <div>
        <h1 className="CrueltyFreePageS5-title">
          Hãy xem chúng ta đang ở đâu hôm nay
        </h1>
        <Row className="CrueltyFreePageS5-container">
          <Col
            span={8}
            style={{ position: "relative", textAlign: "center" }}
            className="CrueltyFreePageS5-col"
          >
            <div className="image-container">
              <img
                src="https://images.ctfassets.net/ywowj8d94i8y/F276sADRPX2sjVhDal86c/37c1af740cfbc2ba9da50879bb133a81/cross_link_cruelty_free_science.png?fm=webp"
                alt=""
                className="CrueltyFreePageS5-image"
              />
            </div>
            <span className="CrueltyFreePageS5-btn-label">
              <p className="CrueltyFreePageS5-text">
                Khoa học Bảo vệ Động vật của P&G
              </p>
              <i className="gg-arrow-right"></i>
            </span>
          </Col>

          <Col
            span={8}
            style={{ position: "relative", textAlign: "center" }}
            className="CrueltyFreePageS5-col"
          >
            <div className="image-container">
              <img
                src="https://images.ctfassets.net/ywowj8d94i8y/F276sADRPX2sjVhDal86c/37c1af740cfbc2ba9da50879bb133a81/cross_link_cruelty_free_science.png?fm=webp"
                alt=""
                className="CrueltyFreePageS5-image"
              />
            </div>
            <span className="CrueltyFreePageS5-btn-label">
              <p className="CrueltyFreePageS5-text">
                Khoa học Bảo vệ Động vật của P&G
              </p>
              <i className="gg-arrow-right"></i>
            </span>
          </Col>

          <Col
            span={8}
            style={{ position: "relative", textAlign: "center" }}
            className="CrueltyFreePageS5-col"
          >
            <div className="image-container">
              <img
                src="https://images.ctfassets.net/ywowj8d94i8y/F276sADRPX2sjVhDal86c/37c1af740cfbc2ba9da50879bb133a81/cross_link_cruelty_free_science.png?fm=webp"
                alt=""
                className="CrueltyFreePageS5-image"
              />
            </div>
            <span className="CrueltyFreePageS5-btn-label">
              <p className="CrueltyFreePageS5-text">
                Khoa học Bảo vệ Động vật của P&G
              </p>
              <i className="gg-arrow-right"></i>
            </span>
          </Col>
        </Row>
        <Row className="CrueltyFreePageS5Mb-container">
          <Col
            span={8}
            style={{ position: "relative", textAlign: "center" }}
            className="CrueltyFreePageS5Mb-col"
          >
            <img
              src="https://images.ctfassets.net/ywowj8d94i8y/F276sADRPX2sjVhDal86c/37c1af740cfbc2ba9da50879bb133a81/cross_link_cruelty_free_science.png?fm=webp"
              alt=""
              className="CrueltyFreePageS5Mb-image"
            />
            <span className="CrueltyFreePageS5Mb-btn-label">
              <p className="CrueltyFreePageS5Mb-text">
                Khoa học Bảo vệ Động vật của P&G
              </p>
              <i className="gg-arrow-right"></i>
            </span>
          </Col>

          <Col
            span={8}
            style={{ position: "relative", textAlign: "center" }}
            className="CrueltyFreePageS5Mb-col"
          >
            <img
              src="https://images.ctfassets.net/ywowj8d94i8y/F276sADRPX2sjVhDal86c/37c1af740cfbc2ba9da50879bb133a81/cross_link_cruelty_free_science.png?fm=webp"
              alt=""
              className="CrueltyFreePageS5Mb-image"
            />
            <span className="CrueltyFreePageS5Mb-btn-label">
              <p className="CrueltyFreePageS5Mb-text">
                Khoa học Bảo vệ Động vật của P&G
              </p>
              <i className="gg-arrow-right"></i>
            </span>
          </Col>

          <Col
            span={8}
            style={{ position: "relative", textAlign: "center" }}
            className="CrueltyFreePageS5Mb-col"
          >
            <img
              src="https://images.ctfassets.net/ywowj8d94i8y/F276sADRPX2sjVhDal86c/37c1af740cfbc2ba9da50879bb133a81/cross_link_cruelty_free_science.png?fm=webp"
              alt=""
              className="CrueltyFreePageS5Mb-image"
            />
            <span className="CrueltyFreePageS5Mb-btn-label">
              <p className="CrueltyFreePageS5Mb-text">
                Khoa học Bảo vệ Động vật của P&G
              </p>
              <i className="gg-arrow-right"></i>
            </span>
          </Col>
        </Row>
      </div>
    </div>
  );
}

export default Section5;
